...
const SignUp = (props) => {
    ...
    const onPasswordChangedHandler = (e) => {
        if (e.target.value !== rePassword) setMessage('Password and Confirm password do not match !');
        else setMessage('');
        setPassword(e.target.value);
    }

    const onRePasswordChangedHandler = (e) => {
        if (password !== e.target.value) setMessage('Password and Confirm password do not match !');
        else setMessage('');
            ...
            <div className="row">
                <div className="col-md-3">
                    <Modal show={coreContext.showPatientConfirmationModal} onHide={coreContext.handlePatientConfirmationModalClose}>
                        ...
                        <Modal.Footer>
                            <Button variant="primary" onClick={() => coreContext.verifyProviderVerificationCode(verificationCode, email, '', '/login')}>
                                Submit
                            </Button>
                    ...
                    <div className="card">
                        ...
                        <div className="card-body">
                            <Form autoComplete='off' onSubmit={handleSubmit(registerUser)} noValidate>
                                ...
                                <Input label='Date of Birth' elementType='date' placeholder='Enter DOB' onChange={onDOBChangedHandler} required={true} minLength={5} maxLength={55} register={register} errors={errors} name='dob' />
                                ...
                                <Input label='Confirm Password' elementType='password' placeholder='Re-enter Password' onChange={onRePasswordChangedHandler} required={true} minLength={5} maxLength={55} register={register} errors={errors} name='password' />

                                <Input disabled={password !== rePassword} blockButton={true} value='Register' elementType='button' variant='primary' />
                                <br />
                                ...
                                <br />
                                <br />
                                <center><a href="login"> Already have an account ? Sign In </a> </center>
                            </Form>
                        </div>
                    </div>
                </div>
